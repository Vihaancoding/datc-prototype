...
def open_selected_drone(drone_id):
    ...
    # Unpack row - qr_content might be NULL
    name, dtype, year, model, manu, qr_active, qr_content, expiry, db_state = row

    # Handle missing QR content
    if not qr_content:
        win.destroy()
        messagebox.showerror(
            "Error",
            f"Drone {drone_id} has no QR code.\n\n"
            "This drone may not have been properly approved yet.\n"
            "QR codes are generated during the approval process."
        )
        return

    config.Companysel = name

    # Extract QR ID from qr_content
    # qr_content format: "http://localhost:5000/verify?id=XXXXX"
    if "verify?id=" in qr_content:
        stripped_id = qr_content.split("verify?id=")[-1]
    else:
        # If qr_content is just the ID itself
        stripped_id = qr_content
